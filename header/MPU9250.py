...
class MPU9250:
    ...
    def configMPU9250(self, gfs, afs):
        ...
        # sleep off
        self.bus_write_byte_data(PWR_MGMT_1, 0x00,1)
        #print("DEBUG: "+self.address)
        time.sleep(0.1)
        # auto select clock source
        self.bus_write_byte_data(PWR_MGMT_1, 0x01,1)
        time.sleep(0.1)
        # DLPF_CFG
        self.bus_write_byte_data(CONFIG, 0x03,1)
        # sample rate divider
        self.bus_write_byte_data(SMPLRT_DIV, 0x04,1)
        # gyro full scale select
        self.bus_write_byte_data(GYRO_CONFIG, gfs << 3,1)
        # accel full scale select
        self.bus_write_byte_data(ACCEL_CONFIG, afs << 3,1)
        # A_DLPFCFG
        self.bus_write_byte_data(ACCEL_CONFIG_2, 0x03,1)
        # BYPASS_EN
        self.bus_write_byte_data(INT_PIN_CFG, 0x02,1)
        time.sleep(0.1)
    ...
    def configAK8963(self, mode, mfs):
        ...
        self.bus_write_byte_data(AK8963_CNTL1, 0x00,0)
        time.sleep(0.01)

        # set read FuseROM mode
        self.bus_write_byte_data(AK8963_CNTL1, 0x0F,0)
        time.sleep(0.01)

        # read coef data
        data = self.bus_read_i2c_block_data(AK8963_ASAX, 3, 1)
        ...
        self.bus_write_byte_data(AK8963_CNTL1, (mfs<<4|mode),0)
        time.sleep(0.01)
    ...
    def checkDataReady(self):
        drdy=self.bus_read_byte_data(INT_STATUS,1)
        if drdy & 0x01:
            return True
        else:
            return False
    ...
    def readAccel(self):
        data = self.bus_read_i2c_block_data(ACCEL_OUT, 6,1)
    ...
    def readGyro(self):
        data = self.bus_read_i2c_block_data(GYRO_OUT, 6, 1)
    ...
    def readMagnet(self):
        ...
        # check data ready
        drdy = self.bus_read_byte_data(AK8963_ST1,0)
        if drdy & 0x01 :
            data = self.bus_read_i2c_block_data(AK8963_MAGNET_OUT, 7, 0)
    ...
    def readTemperature(self):
        data = self.bus_read_i2c_block_data(TEMP_OUT, 2,1)
    ...
    def bus_write_byte_data(self,setParameter,register_number,select):
        run=False
        if select is 1:
            address=self.address
        else:
            address=AK8963_SLAVE_ADDRESS
        while(not run):
            try:
                bus.write_byte_data(address, setParameter, register_number)
                run=True
    ...
    def bus_read_byte_data(self,setParameter,select):
        run=False
        if select is 1:
            address=self.address
        else:
        ...
        while(not run):
            try:
                result=bus.read_byte_data(address, setParameter)
                run=True
                return result
    ...
    def bus_read_i2c_block_data(self,setParameter,parameter,select):
        run=False
        if select is 1:
            address=self.address
        else:
        ...
        while(not run):
            try:
                result=bus.read_i2c_block_data(address, setParameter, parameter)
                run=True
                return result
